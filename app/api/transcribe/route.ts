...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existsSync(subtitlePath)) {
      const existingSubtitles = JSON.parse(
        await readFile(subtitlePath, "utf-8")
      );
      return NextResponse.json({
        success: true,
    ...
    const videoPath = path.join(storageDir, videoId);
    ...
    if (!existsSync(scriptPath)) {
      return NextResponse.json(
        {
          error:
            "Transcription script not found. Please ensure scripts/transcribe.py exists.",
        },
        { status: 500 }
      );
    }
    ...
    console.log(
      `Running local Whisper transcription with model: ${model}, language: ${
        language || "auto"
      }, translate: ${translate || false}`
    );

    try {
      const args = [scriptPath, videoPath, "--model", model];
      ...
      const { stdout, stderr } = await execFileAsync(pythonExec, args, {
      ...
      if (execError.killed || execError.signal === "SIGTERM") {
        return NextResponse.json(
          {
            error:
              "Transcription timed out. The video may be too long or the model too slow. Try using a smaller model (tiny, base) or shorter video.",
          },
          { status: 500 }
        );
      }
